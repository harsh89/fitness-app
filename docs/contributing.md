...
- __Files__
  ...
  - With the exception of Containers and Components, which should be `PascalCase` - eg. `ChallengeView.js`
- __Directories__
...
- If there's more than a few files in a directory that are related, group them within their own directory
  - eg. if I have 2 components: `/components/ChallengeListing.js` and `/components/ChallengeView.js`, I may choose to create a new directory within components called `challenges` and put the 2 files within (removing `Challenges`). The result would be: `/components/challenges/Listing.js` and `/components/challenges/View.js`
